...
const app = {
  ...
  preferences: {font:'system',boardTheme:'classic',background:'none',pieceStyle:'traditional',musicStyle:'fc',volume:70},
  options: {}, profiles: new Map(), pieceImages: new Map(), animation: null, legalPromise: null, resultKey: null,
};
...
function saveCurrentReplay(){const document=currentGameDocument();if(!document)return toast('当前没有可保存的棋局');const records=replayRecords();records.unshift({id:Date.now(),savedAt:document.savedAt,document});localStorage.setItem('xh-replays',JSON.stringify(records.slice(0,30)));renderSavedReplays();toast('棋局已保存到本机棋谱库')}
...
function canControl(piece){return piece&&piece.color===app.state.turn&&(app.mode==='local'||piece.color===app.color)}
...
window.addEventListener('resize',resizeBoard);$('#newGameButton').onclick=createGame;$('#joinButton').onclick=joinGame;$('#resignButton').onclick=()=>send('resign');$('#drawButton').onclick=()=>send('draw_offer');$('#undoButton').onclick=()=>send('undo_request');$('#restartButton').onclick=()=>send('restart',{options:app.options});$('#replayButton').onclick=()=>openReplay();
