...
from PySide6.QtWidgets import (
    ...
    QInputDialog, QLineEdit, QFileDialog, QTextBrowser, QSlider, QTabWidget,
)
...
class AIWorker(QRunnable):
    ...
    def run(self) -> None:
        try:
            self.signals.finished.emit(self.ai.choose_move(self.game, self.cancel))
...
class BoardWidget(QWidget):
    ...
    def set_game(self, game: Game, human_color: Color) -> None:
        self.game, self.human_color = game, human_color
        self.selected, self.legal, self.capturable = None, [], set()
        self.update()

    def set_appearance(self, theme_name: str, background_name: str) -> None:
    ...
    def paintEvent(self, event) -> None:
        ...
        if rows == 13:
            ...
            for row in range(rows):
                painter.drawLine(QPointF(left, top + row * ch), QPointF(left + (cols - 1) * cw, top + row * ch))
            for col in range(cols):
                painter.drawLine(QPointF(left + col * cw, top), QPointF(left + col * cw, top + (rows - 1) * ch))
    ...
    def _draw_piece(self, painter, piece, left, top, cw, ch):
        ...
        painter.setBrush(QBrush(gradient)); painter.setPen(QPen(QColor("#9a2424") if piece.color is Color.RED else QColor("#24211e"), max(2, radius*.1)))
    ...
    def mousePressEvent(self, event) -> None:
        if event.button() is Qt.MouseButton.LeftButton and not self.locked and self.interactive and self.game:
            ...
            if piece and piece.color is self.human_color and piece.color is self.game.state.turn:
...
class SettingsDialog(QDialog):
    def __init__(self, config: dict, parent=None):
        ...
        self.theme=QComboBox();[(self.theme.addItem(text,value)) for value,text in (("classic","经典木纹"),("green","翠绿"),("blue","海洋蓝"),("dark","暗黑"))];self.theme.setCurrentIndex(max(0,self.theme.findData(config.get("theme","classic"))))
        ...
        self.background=QComboBox();[(self.background.addItem(f"背景 {index}",str(index))) for index in range(1,6)];self.background.setCurrentIndex(max(0,self.background.findData(str(config.get("background","1")))))
        ...
        game_form.addRow("模式",self.profile);game_form.addRow("AI 难度",self.difficulty)
        game_form.addRow("玩家执子",self.color);game_form.addRow("每方时间（分钟）",self.minutes)
        ...
        appearance_form.addRow("背景图片",self.background);appearance_form.addRow("音效",self.sound)
        ...
        self.checks={}; options=PROFILES[config["profile"]].options.merged(config.get("rule_options",{}))
        ...
        self.profile.currentIndexChanged.connect(self._update_piece_availability);self._update_piece_availability()
    ...
    def value(self) -> dict:
        values={k:c.isChecked() for k,c in self.checks.items()};values["king_appear"]=True
        return {"profile":self.profile.currentData(),"difficulty":self.difficulty.currentData(),"human_color":self.color.currentData(),"initial_minutes":self.minutes.value(),"sound":self.sound.isChecked(),"music":self.music.isChecked(),"music_style":self.music_style.currentData(),"sound_volume":self.sound_volume.value(),"music_volume":self.music_volume.value(),"theme":self.theme.currentData(),"font":self.font.currentData(),"background":self.background.currentData(),"autosave":self.autosave.isChecked(),"server_url":self.parent().config.get("server_url","http://127.0.0.1:8000") if self.parent() else "http://127.0.0.1:8000","rule_options":values}
...
class ReplayDialog(QDialog):
    def __init__(self, game: Game, config: dict, parent=None):
        ...
        self.board.set_appearance(config.get("theme","classic"), str(config.get("background","1")))
    ...
    def show_step(self, step: int) -> None:
        ...
        self.board.set_game(replay,Color.RED);self.board.interactive=False
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.resize(1260,850);self.setMinimumSize(920,680);self._build();self.new_game()
    ...
    def _build(self):
        ...
        file_menu.addSeparator();autosaves=QAction("打开自动保存目录",self);autosaves.triggered.connect(self.open_autosave_folder);file_menu.addAction(autosaves)
        game_menu = self.menuBar().addMenu("对局")
        for title, callback, shortcut in (("新建对局",self.new_game,"Ctrl+N"),("悔棋",self.undo_move,"Ctrl+Z"),("复盘",self.replay_game,"Ctrl+R")):
        ...
        network_menu = self.menuBar().addMenu("联机")
        for title, callback in (("创建局域网房间", self.create_online), ("加入局域网房间", self.join_online)):
            action = QAction(title, self); action.triggered.connect(callback); network_menu.addAction(action)
        ...
        fullscreen=QAction("全屏",self);fullscreen.setShortcut(QKeySequence("F11"));fullscreen.triggered.connect(self.toggle_fullscreen);settings_menu.addAction(fullscreen)
        ...
        side=QWidget();side.setMaximumWidth(330);right=QVBoxLayout(side);self.black=QLabel();self.red=QLabel();self.history=QListWidget();self.captured=QLabel();self.captured.setWordWrap(True)
        ...
        actions=QGridLayout();self.undo=QPushButton("悔棋");self.undo.clicked.connect(self.undo_move);self.resign=QPushButton("认输");self.resign.clicked.connect(self.resign_game);self.draw=QPushButton("提和");self.draw.clicked.connect(self.draw_game);self.resurrect=QPushButton("复活兵卒");self.resurrect.clicked.connect(self.resurrect_pawn)
        for i,b in enumerate((self.undo,self.resign,self.draw,self.resurrect)):actions.addWidget(b,i//2,i%2)
        right.addLayout(actions);split.addWidget(side);split.setSizes([850,300])
        self.apply_appearance()

    def new_game(self):
        self.close_network();self.cancel.set();self.cancel=threading.Event();self.game=Game(self.config["profile"],self.config.get("rule_options"),self.config["initial_minutes"]);human=Color(self.config["human_color"]);self.board.set_game(self.game,human);self.board.set_appearance(self.config.get("theme","classic"),str(self.config.get("background","1")));self.board.locked=False;self._stats_recorded=False;self._autosaved=False;self.refresh();
        if self.game.state.turn is not human:self.start_ai()
    ...
    def human_move(self, move: Move):
        ...
        try:
            ...
            self.game.move(move);self.play_sound();self.refresh();
            if not self.game.state.finished:self.start_ai()
        except GameError as exc:QMessageBox.warning(self,"非法走棋",str(exc))

    def start_ai(self):
        if self.board.locked or self.game.state.finished:return
    ...
    def finish_ai(self, move):
        ...
        try:
            if move:self.game.move(move);self.play_sound()
        except GameError as exc:self.ai_failed(str(exc));return
        self.board.locked=False;self.refresh()

    def ai_failed(self,message):self.board.locked=False;self.refresh();QMessageBox.warning(self,"AI 异常",message)
    def play_sound(self):
    ...
    def undo_move(self):
        ...
        try:self.cancel.set();self.game.undo(2 if len(self.game.state.history)>=2 else 1);self.cancel=threading.Event();self.board.locked=False;self.refresh()
        except GameError as exc:QMessageBox.information(self,"悔棋",str(exc))
    def resign_game(self):
        if self.network:self.send_network("resign");return
        self.game.resign(Color(self.config["human_color"]));self.refresh()
    def draw_game(self):
        if self.network:self.send_network("draw_offer");return
        if QMessageBox.question(self,"提和","离线人机模式中确认以和棋结束？")==QMessageBox.StandardButton.Yes:self.game.state.draw=True;self.game.state.result_reason="draw_agreement";self.refresh()
    ...
    def resurrect_pawn(self):
        color=Color(self.config["human_color"]);row=8 if color is Color.RED else 4
        for col in range(0,self.game.profile.cols,2):
            try:self.game.resurrect_pawn(color,Position(row,col));self.refresh();self.start_ai();return
    ...
    def apply_appearance(self):
        ...
        QApplication.instance().setFont(QFont(font,10));self.board.set_appearance(self.config.get("theme","classic"),str(self.config.get("background","1")))
    ...
    def show_about(self):
        DocumentDialog("关于匈汉象棋","# 匈汉象棋 3.0\n\nPySide6 桌面端、Web 端与 Android 端共用 Python 规则、AI 和对局模型。\n\n本版本参考原 Desktop 的菜单、音频、复盘、棋谱和统计功能重新实现。",self).exec()
    ...
    def refresh(self):
        ...
        state=self.game.state;self.banner.setText("对局结束" if state.finished else f"{'红方' if state.turn is Color.RED else '黑方'}行棋")
