...
class Game:
    def __init__(self, profile: RuleProfile | str = "desktop_complete",
        ...
        self.state = GameState(self.profile.id, self.profile.initial_pieces(self.options))
        self.state.clocks_ms = {Color.RED: initial_minutes * 60_000, Color.BLACK: initial_minutes * 60_000}
        self._snapshots: list[dict] = []
        self._record_position()

    @classmethod
    def from_state(cls, state: GameState, option_overrides: dict[str, object] | RuleOptions | None = None) -> "Game":
        if isinstance(option_overrides, RuleOptions):
            option_overrides = asdict(option_overrides)
        game = cls(state.profile_id, option_overrides)
        game.state = state
        return game
    ...
    def notation(self, piece: Piece, move: Move, captured: tuple[Piece, ...]) -> str:
        name = self.profile.name_of(piece)
        action = "吃" if captured else "至"
