...
本轮已执行以下 P1 方案，并通过全量回归：

- **A-2 旧 `.fen` 兼容导入**：新增 `core/legacy.py`，支持传统 10×9、匈汉 13×13 的纯 FEN，以及旧版 JSON 包装格式；统一迁移为 `GameDocument v1`，桌面、Web 与 Android 离线导入入口均接受 `.fen`。Android 同时修复载入已结束存档后重复计入统计的问题。
- **B-2 联机重新开局双方确认**：协议新增 `restart_request` / `restart_response`，在线房间必须由对手接受后才重置棋局；本地和 AI 模式继续保持立即重开。
- **B-3 统计细分**：桌面和 Web 增加累计时长、最快胜局、当前/最高连胜及分色统计，并向后兼容旧统计文件。

验证结果：`86 passed`，JavaScript 语法、locale JSON、Python compileall 与 `git diff --check` 全部通过。

## 2026-08-13 后续迭代进展

- **C-1 最近一步轨迹动画**：桌面、Web、Android 离线端统一显示起点到终点的连线、箭头与淡出效果；保留现有动画开关。
- **C-2 将军脉冲提示**：桌面王位上方显示脉冲“将军”，Web 回合条显示将军并缩放提示，Android 离线端显示一次性将军提示；均沿用既有音效防重复逻辑。
- **B-1 完整挑衅系统（第一阶段）**：新增开局、将军、胜利、失败四类共 16 条以上语料；桌面快捷短语和 Web 快捷短语已扩展。AI 自动事件触发与独立设置开关列入下一阶段。
- **A-3 头像系统（轻量阶段）**：Web 玩家区根据玩家名称显示稳定默认头像首字符，并同步更新玩家名称；自定义头像 URL、账号存储字段和桌面/Android 上传入口列入下一阶段。
- **A-1 MCTS（第一阶段）**：新增无第三方依赖、可取消的纯 UCB1 推理模块与 pytest 覆盖；尚未替换现有 Negamax/Alpha-Beta 默认档位，也未开始神经网络训练流水线。
- **B-1 完整挑衅系统（第二阶段）**：AI 在开局、将军、胜利和失败事件后自动发送场景化短语；服务端保存最近 20 条自动消息并按事件去重，Web 与桌面提供独立开关。玩家手动快捷短语保持可用。
- **A-3 头像系统（第二阶段）**：账号数据库新增可选 `avatar_url` 并自动迁移旧库；新增资料更新 API，房间快照携带头像 URL。Web 支持编辑和渲染 URL 头像，Android 联机页复用 Web 渲染，桌面登录资料会随创建/加入房间传递。

本轮新增回归后全量测试为 `91 passed`，唯一警告仍为 Starlette/httpx 测试客户端弃用提示。

按本阶段约束暂缓：MCTS 接入默认难度档位、自对弈/神经网络训练流水线。头像文件上传与裁剪、桌面头像图片展示、Android 离线身份资料仍需后续独立 UI 与存储验收。
