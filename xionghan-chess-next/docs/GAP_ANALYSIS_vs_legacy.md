...
本轮新增回归后全量测试为 `89 passed`，唯一警告仍为 Starlette/httpx 测试客户端弃用提示。

尚未实施：A-1 MCTS、自对弈训练流水线、A-3 头像、B-1 挑衅系统、C-1 轨迹动画、C-2 将军脉冲提示。这些项目需要独立的 UI/性能验收周期，不适合与存储和协议变更混在一次迭代中。
