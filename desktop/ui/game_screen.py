...
class BaseGameScreen:
    ...
    @staticmethod
    def draw_captured_pieces(screen, game_state, window_width):
        """绘制双方阵亡棋子"""
        # 绘制标题
        ...
        # 将阵亡棋子信息移到右侧
        right_panel_x = window_width - 250  # 右侧边栏起始x坐标
        ...
        for config in configurations:
            ...
            for piece in game_state.captured_pieces[config["color"]]:
                ...
                # 减小右边距，提供更多空间给棋子显示
                if x + piece_text.get_width() > window_width - 40:
                    x = config["x_start"]
                    y += 25
    ...
    @staticmethod
    def draw_move_history(screen, game_state, window_width):
        """绘制棋谱历史记录"""
        # 只显示最近的棋谱记录
        if hasattr(game_state, 'move_history') and game_state.move_history:
            ...
            screen.blit(history_title, (window_width - 250, 300))
            ...
            for i, move_record in enumerate(recent_moves):
                ...
                # 绘制文本
                screen.blit(text_surface, (window_width - 250, start_y + i * line_spacing))
...
class GameScreen(BaseGameScreen):
    ...
    def handle_event(self, event, mouse_pos, game):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # 检查是否点击了音效设置按钮
                ...
                # 打开音效设置对话框
                from desktop.ui.dialogs import AudioSettingsDialog
                game.audio_settings_dialog = AudioSettingsDialog(game.sound_manager)
    ...
    def handle_menu_events(self, event, mouse_pos, game, game_state):
        ...
        if option_result == "toggle":
            ...
            from desktop.controllers.game_io_controller import game_io_controller
            if game_io_controller.import_game(game_state):
                # 进入复盘模式
                from desktop.controllers.replay_controller import ReplayController
                from desktop.ui.replay_screen import ReplayScreen
            ...
            # 打开统计数据界面
            from desktop.ui.dialogs import StatisticsDialog
        ...
        if help_result == "toggle":
            ...
            # 打开游戏规则界面
            from desktop.ui.rules_screen import RulesScreen
            rules_screen = RulesScreen()
            rules_screen.run()
            return "handled"
        elif help_result == "关于":
            # 打开关于界面，显示作者信息
            from desktop.ui.about_screen import AboutScreen
    ...
    def handle_operation_panel_events(self, event, mouse_pos, game):
        ...
        if op_result == "toggle":
            ...
            from desktop.ui.dialogs import ConfirmDialog
