...
from desktop.utils.tools import load_font
from desktop.controllers.game_config_manager import get_piece_color, get_piece_text_color, theme_manager, THEME_CONFIG


class ChessBoard:
    def __init__(self, window_width, window_height, margin_left, margin_top, traditional_mode=False):
        """初始化棋盘
        ...
        Args:
            ...
            margin_top (int): 顶部边距
            traditional_mode (bool): 是否使用传统象棋模式
        """
    ...
    def draw(self, screen, pieces, game_state=None):
        ...
        if self.traditional_mode:
            self._draw_traditional_chess_board(screen, theme_colors)
        else:
            self._draw_hungarian_chinese_chess_board(screen, theme_colors)
        ...
        # 绘制兵/卒复活的小星星标记
        if game_state and not self.traditional_mode:
        ...
        if self.highlighted:
            ...
            if self.traditional_mode:
                x = self.margin_left + col * (self.board_width / 8)
                y = self.margin_top + row * (self.board_height / 9)
                highlight_radius = (self.board_width / 8) * 0.4
            else:
                x = self.margin_left + col * self.grid_size
                y = self.margin_top + row * self.grid_size
                highlight_radius = self.grid_size * 0.4
    ...
    def _draw_hungarian_chinese_chess_board(self, screen, theme_colors):
        """绘制匈汉象棋棋盘"""
        # 绘制棋盘背景
        board_rect = pygame.Rect(
            self.margin_left, 
               ...
               game_state.get_piece_at(row, col) is None:
                ...
                if game_state.get_pawn_count(current_player) < 7:
                    ...
                    if (row, col) in resurrection_positions[current_player]:
                        # 弹出复活确认对话框
                        from desktop.ui.dialogs import PawnResurrectionDialog
        ...
        if game_instance.selected_piece is None:
            ...
            if move_successful:
                ...
                if game_state.needs_promotion:
                    ...
                    # 自动弹出升变选择对话框
                    from desktop.ui.dialogs import PromotionDialog
                ...
                if game_instance.selected_piece and not captured_piece:
                    try:
                        from desktop.controllers.sound_manager import sound_manager
