...
class ChessGame:
    ...
    def __init__(self, game_mode: str = MODE_PVP, player_camp: str = CAMP_RED, game_settings: Optional[Dict[str, Any]] = None):
        ...
        # 初始化AI管理器（如果需要）
        from desktop.controllers.ai_manager import AIManager
        ...
        # 将军/绝杀提示管理器
        from desktop.controllers.check_checkmate_tip_manager import CheckCheckmateTipManager
    ...
    def _execute_move_with_command_pattern(self, move: Tuple[int, int, int, int], 
        ...
        Returns:
            bool: 是否执行成功
        """
        from desktop.core.commands import MoveData
        from desktop.core.xionghan_move_command import XionghanMoveCommand
    ...
    def restart_game(self):
        ...
        # 重置步数计数器
        from desktop.controllers.step_counter import step_counter
        step_counter.reset()
    ...
    def run(self):
        ...
        while True:
            ...
            if self.game_state.game_over and not self.state_machine.is_in_state(self.state_machine.__class__.__dict__.get('GAME_OVER')):
                from desktop.core.game_state_machine import GamePhase
            ...
            for event in pygame.event.get():
                ...
                # 如果有统计数据对话框，处理它的事件
                    ...
                    if result == "close":
                        ...
                        # 重置统计数据
                        from desktop.controllers.statistics_manager import statistics_manager
