...
def load_theme_config():
    ...
    try:
        ...
        theme_config_path = os.path.join(current_dir, "..", "assets", "docs", "theme_config.json")
        ...
        if os.path.exists(theme_config_path):
            ...
            # 如果外部配置文件不存在，返回默认配置
            print(f"主题配置文件不存在: {theme_config_path}，使用默认配置")
            return get_default_theme_config()
    except Exception as e:
        print(f"加载主题配置失败: {e}，使用默认配置")
        return get_default_theme_config()

def get_default_theme_config():
    """获取默认主题配置"""
    return {
        "origin": {  # 原始主题
            "background": [220, 179, 92],  # 背景颜色（棋盘木色）
...
def get_piece_text_color(theme: str, side: str) -> tuple:
    """获取棋子文字配色"""
    try:
        color = THEME_CONFIG[theme]["pieces"][side]["text_color"]
        return tuple(color) if isinstance(color, list) else color
    except KeyError:
        # 处理主题或阵营不存在的情况
    ...
    def _load_settings_from_file(self):
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                loaded_settings.update(file_config)
