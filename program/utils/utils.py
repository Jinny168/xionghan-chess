...
def load_sound(sound_name):
    ...
    sound_paths = {
        ...
        'jiangjun_voice': os.path.join("sounds", "jiangjun.wav"),
        'juesha_voice': os.path.join("sounds", "juesha.wav"),
        'victory': os.path.join("sounds", "fc_victory_sound.mp3"),  # 默认使用fc风格的胜利音效
        'defeat': os.path.join("sounds", "fc_defeat_sound.mp3"),    # 默认使用fc风格的失败音效
        'qq_victory': os.path.join("sounds", "qq_victory_sound.wav"),
        'qq_defeat': os.path.join("sounds", "qq_defeat_sound.wav"),
        'fc_victory': os.path.join("sounds", "fc_victory_sound.mp3"),
        'fc_defeat': os.path.join("sounds", "fc_defeat_sound.mp3")
    }
...
class SoundManager:
    ...
    def play_background_music(self):
        ...
        if self.current_music_style == 'qq':
            music_filename = "qq_background_sound.mp3"
        else:
            music_filename = "fc_background_sound.mp3"
            
        music_filepath = resource_path(os.path.join("sounds", music_filename))
        ...
        if os.path.exists(music_filepath):
            ...
            print(f"警告: 背景音乐文件 {music_filename} 不存在")
