...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        #     counta = torch.sum(value>=0.995).item()
        #     policy_mean = policy.mean()
        #     policy_var = policy.var()
        #     countb = torch.sum(value<=-0.995).item()
        #     x=x
...
class PolicyValueNet:
    ...
    def policy_value_fn(self, board):
        ...
        log_act_probs, value = log_act_probs.cpu() , value.cpu()
        act_probs = np.exp(log_act_probs.numpy().flatten()) if CONFIG['use_frame'] == 'paddle' else np.exp(log_act_probs.detach().numpy().astype('float32').flatten())
        #print('max act_probs:{}({}), value:{} '.format(np.max(act_probs), np.argmax(act_probs), value))
        # 只取出合法动作
    ...
    def train_step(self, state_batch, mcts_probs, winner_batch, lr=0.002):
        ...
        return loss.detach().cpu().numpy(), entropy.detach().cpu().numpy(), value_loss, policy_loss
